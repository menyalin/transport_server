...
export function parsePeriod(period) {
    ...
    if (period.some(date => !dayjs(date).isValid())) 
        throw new Error('AutoSetRouteDatesDTO: invalid params: period dates not valid')
    return period.map(p => new Date(p)).sort((a, b) => a - b)
}


export class AutoSetRouteDatesDTO {
    constructor ({truckIds, period, tripDurationInMinutes, unloadingDurationInMinutes }){
        truckIdsValidator( truckIds )
        this._truckIds = truckIds
        this._period = parsePeriod(period)
        if (Number.isNaN(Number.parseInt(tripDurationInMinutes)) 
        ...
        this._unloadingDurationInMinutes = Number.parseInt(unloadingDurationInMinutes)
    }
